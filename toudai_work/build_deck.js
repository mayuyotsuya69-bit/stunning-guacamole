...
function header(s,kicker,title){
  ...
  s.addText(kicker,{x:0.6,y:0.34,w:12,h:0.4,fontFace:W6,fontSize:13,color:OR,charSpacing:1});
  s.addText(title,{x:0.6,y:0.66,w:12.2,h:0.7,fontFace:W6,fontSize:24,color:DK});
...
s.addShape(p.ShapeType.rect,{x:0,y:0,w:13.333,h:7.5,fill:{color:WH}});
...
foot(s,"出典：過去答案 n=36（合格19/不合格17）。重複答案は最新版を採用。n小につき点推定は高くてもCIは広く、確証には今後のサンプル追加が必要（誠実注記）。");

/* ---------- Slide 2: ② 増分妥当性 ---------- */
s=p.addSlide(); header(s,"② 増分妥当性 ｜ 循環論法の排除","作文スコアを「超えて」合否を予測する");
s.addImage({path:"fig2_incremental.png",x:0.55,y:1.95,w:5.9,h:3.76,shadow:sh()});
bullets(s,6.7,1.85,6.2,[
 {t:"東大スコアは作文スコアを超えて合否を上乗せ予測：尤度比検定 χ²(1)=6.0, p=0.014",bold:true,fs:14,c:DK},
 {t:"逆に、作文スコアは東大スコアを超える上乗せ予測なし：χ²(1)=2.4, p=0.124",fs:13},
 {t:"汎化性能(LOO交差検証) AUC：東大スコア 0.79 ＞ 作文スコア 0.55（ほぼ偶然）",fs:13},
]);
s.addShape(p.ShapeType.rect,{x:6.7,y:4.35,w:6.05,h:2.1,fill:{color:LT},line:{color:AC,width:1}});
s.addText([{text:"なぜ重要か　",options:{fontFace:W6,fontSize:13,color:DK}},
 {text:"東大スコアは作文スコアと相関0.78。「単なる作文点の言い換え」なら妥当性の証明にならない。",options:{fontFace:W3,fontSize:12.5,color:BK,breakLine:true,paraSpaceAfter:4}},
 {text:"本検証は、作文点を統計的に差し引いてもなお合否を予測する=東大推薦に固有のシグナルを捉えていることを示す。",options:{fontFace:W3,fontSize:12.5,color:BK}}],
 {x:6.9,y:4.5,w:5.7,h:1.85,valign:"top"});
foot(s,"出典：過去答案 n=36。ロジスティック回帰（合否～東大スコア／作文スコア／両方）。LOOは leave-one-out 交差検証AUC。");
...
foot(s,"出典：現役 n=582（学年589・高校ランク582・校舎374で実施）。性別は提供データに無し。偏相関は作文AIスコアを共変量に統制。");
...
s.addText([
 ...
 {text:"・ 作文スコアを統計的に差し引いてもなお合否を上乗せ予測（p=0.014）。単なる作文点ではない。",options:{fontFace:W3,fontSize:16,color:WH,breakLine:true,paraSpaceAfter:10}},
 {text:"・ 属性差（学年・校舎等）の正体は概ね作文力。本指標は合否に固有のシグナルを捉えている。",options:{fontFace:W3,fontSize:16,color:WH}},
],{x:0.95,y:3.2,w:11.5,h:2.2,valign:"top"});
s.addShape(p.ShapeType.rect,{x:0.9,y:6.05,w:11.5,h:0.9,fill:{color:AC}});
s.addText("誠実注記：過去検証は n=36 と小規模。点推定は高いが信頼区間は広く、確証には今後の合否サンプル追加で再検証する。",
 {x:1.1,y:6.13,w:11.1,h:0.74,fontFace:W3,fontSize:12.5,color:WH,valign:"middle"});
